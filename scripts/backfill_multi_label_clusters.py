"""
Backfill role_clusters_json (multi-label) for all active jobs with embeddings.
Run once after applying migration 010.

Usage:
    uv run python scripts/backfill_multi_label_clusters.py
"""
...
from mcf.lib.classifiers import classify_jobs_multilabel
...
WHERE j.is_active = TRUE
  ...
  AND j.role_clusters_json IS NULL
"""
...
while True:
    ...
    print(f"  Processed {total:,} jobs  ({total/elapsed:.0f} jobs/s)", end="\r")

conn.close()

elapsed = time.perf_counter() - t0
print(f"\nDone. Processed {total:,} jobs in {elapsed:.1f}s ({total/elapsed:.0f} jobs/s)")
